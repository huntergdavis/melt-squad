export interface BalancePlan {
  ...
  arm: number;
  left: { target?: string; mass: number; arm?: number };
  right: { target?: string; mass: number; arm?: number };
}
...
export function stepBalance(
  ...
  state.angle = Number.isFinite(state.angle)
    ? clamp(state.angle, -MAX_ANGLE, MAX_ANGLE) : 0;
  state.velocity = Number.isFinite(state.velocity)
    ? clamp(state.velocity, -20, 20) : 0;
  state.settled = Number.isFinite(state.settled)
    ? clamp(state.settled, 0, REST_HOLD) : 0;
  state.leftMass = left;
  ...
  const inertia = ballastMass * centerOffset ** 2 + left * L ** 2 + right * R ** 2;
  const loadTorque = right * R - left * L;
  const torque =
    loadTorque * gravity * Math.cos(state.angle) - restoring * Math.sin(state.angle);
  ...
  const matched = validArms && left + right > 0.05 && Math.abs(loadTorque) < 0.02;
  const resting =
    Math.abs(state.angle) < 0.018 && Math.abs(state.velocity) < 0.025;
  state.settled = matched && resting ? Math.min(REST_HOLD, state.settled + dt) : 0;
